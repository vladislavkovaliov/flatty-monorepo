...
export function MicrofrontendHost({
  ...
  useEffect(() => {
    ...
    // biome-ignore lint/suspicious/noConfusingVoidType: must match LoaderUtils.loadScript() return type
    const loadBundleScript = (): Promise<ScriptLoadError | void> => {
      if (bundleName === APPLICATION_BUNDLE_NAME.APP) {
        const baseUrl = `${window.location.origin}${proxyBasePath}`;

        return LoaderUtils.loadScript({
          src: `${baseUrl}/${bundleName}.js`,
          params: {},
        });
      }
  ...
  }, [hostEl, bundleName, proxyBasePath, remoteOrigin, config, basePath]);
