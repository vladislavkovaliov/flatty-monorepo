...
type StatsUpdater interface {
	...
	GetTotal(ctx context.Context, residentLocationID int64, month, year int) (float64, error)
	GetAverage(ctx context.Context, residentLocationID int64, month, year int) (float64, int, error)
}
...
func (r *statsRepo) GetTotal(ctx context.Context, residentLocationID int64, month, year int) (float64, error) {
	var total float64
	err := r.pool.QueryRow(ctx, `SELECT total_spent FROM expense_monthly_totals WHERE resident_location_id = $1 AND month = $2 AND year = $3`, residentLocationID, month, year).Scan(&total)
	if err != nil {
		return 0, err
	}
	return total, nil
...
func (r *statsRepo) GetAverage(ctx context.Context, residentLocationID int64, month, year int) (float64, int, error) {
	var avg float64
	var cnt int
	err := r.pool.QueryRow(ctx, `SELECT average_amount, expense_count FROM expense_monthly_averages WHERE resident_location_id = $1 AND month = $2 AND year = $3`, residentLocationID, month, year).Scan(&avg, &cnt)
	if err != nil {
		return 0, 0, err
	}
	return avg, cnt, nil
...
func (r *statsRepo) UpsertTotal(ctx context.Context, residentLocationID int64, month, year int, totalSpent float64) error {
...
func (r *statsRepo) UpsertAverage(ctx context.Context, residentLocationID int64, month, year int, averageAmount float64, expenseCount int) error {
	_, err := r.pool.Exec(ctx, `
...
func (c *Consumer) handleCreated(ctx context.Context, event ExpenseEvent) error {
	prevTotal, _ := c.stats.GetTotal(ctx, event.ResidentLocationID, event.Month, event.Year)
	newTotal := prevTotal + event.Amount
	if err := c.stats.UpsertTotal(ctx, event.ResidentLocationID, event.Month, event.Year, newTotal); err != nil {
		return fmt.Errorf("upsert total: %w", err)
	}

	prevAvg, prevCount, _ := c.stats.GetAverage(ctx, event.ResidentLocationID, event.Month, event.Year)
	newCount := prevCount + 1
	newAvg := newTotal / float64(newCount)
	if prevCount == 0 {
		newAvg = event.Amount
	}
	if err := c.stats.UpsertAverage(ctx, event.ResidentLocationID, event.Month, event.Year, newAvg, newCount); err != nil {
...
func (c *Consumer) handleUpdated(ctx context.Context, event ExpenseEvent) error {
	...
	_, prevCount, _ := c.stats.GetAverage(ctx, event.ResidentLocationID, event.Month, event.Year)
	if prevCount > 0 {
		newAvg := newTotal / float64(prevCount)
		if err := c.stats.UpsertAverage(ctx, event.ResidentLocationID, event.Month, event.Year, newAvg, prevCount); err != nil {
			return fmt.Errorf("upsert average: %w", err)
		}
	}
