...
export function SpendingPage() {
  ...
  const totals = useMemo(
    () => totalsData?.expenseMonthlyTotals?.data ?? [],
    [totalsData?.expenseMonthlyTotals?.data],
  );
  const averages = useMemo(
    () => averagesData?.expenseMonthlyAverages?.data ?? [],
    [averagesData?.expenseMonthlyAverages?.data],
  );
  ...
  const minMax = useMemo(() => {
    ...
    for (const row of merged) {
      if (
        row.year < minYear ||
        (row.year === minYear && row.month < minMonth)
      ) {
        minYear = row.year;
        minMonth = row.month;
      }
      if (
        row.year > maxYear ||
        (row.year === maxYear && row.month > maxMonth)
      ) {
        maxYear = row.year;
        maxMonth = row.month;
      }
  ...
  const filtered = useMemo(() => {
    ...
    if (appliedFrom) {
      ...
      result = result.filter(
        (r) =>
          r.year > fromYear || (r.year === fromYear && r.month >= fromMonth),
      );
    }
