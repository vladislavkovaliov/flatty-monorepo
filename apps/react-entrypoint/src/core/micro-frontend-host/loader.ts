...
export const LoaderUtils = {
  ...
  isStyleLoaded(href: string): boolean {
  ...
  loadScript({
    src,
    params: { defer = false, async = true, ...otherProps } = {},
  }: ILoadScriptProps): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.addEventListener("error", (event) => {
        ...
        reject(
          new Error(
            `Failed to load script: ${src}${cause ? ` (${String(cause)})` : ""}`,
            {
              cause: event,
            },
          ),
  ...
  loadStyle(href: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      link.addEventListener("error", (event) => {
        reject(
          new Error(`Failed to load stylesheet: ${href}`, { cause: event }),
        );
      });
